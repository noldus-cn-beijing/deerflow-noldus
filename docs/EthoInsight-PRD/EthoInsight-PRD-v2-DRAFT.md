...
3. **判断结果是否正常** — "开放臂停留时间 12%" 到底算正常还是偏高？不同品系、不同实验条件下的参考范围是什么？
...
**EthoInsight 的差异化**：不是又一个统计工具，而是将 Noldus 数十年的行为学领域知识编码为结构化知识库，由 AI 自动应用到每一次分析中。广东一湾的 EthoClaw 已率先推出针对传统范式的自动化分析 Skills，建立了先发优势——EthoInsight 需要以更深厚的领域知识编码（Noldus 几十年的行为学专业积累）和更强的分析深度（统计方法选择、文献阈值参照、跨范式综合）来建立差异化壁垒。
...
| 新奇抑制摄食 (NSF) | 摄食潜伏期、接近次数 | 潜伏期↑ 提示焦虑样行为↑ |
...
| 糖水偏好 (SPT) | 糖水偏好率、液体总消耗量 | 偏好↓ 提示抑郁样行为↑ |

> **参考品系**: C57BL/6J 成年雄性小鼠，标准化实验条件。各指标的参考范围基于已发表文献（详见附录 A），用于辅助用户解读组间比较结果。
...
> **US-2.3**: 作为研究人员，我希望系统基于已发表文献的参考阈值，自动对焦虑/抑郁水平进行分级（正常/轻度/中度/重度），这样我能快速判断实验动物的表型。
>
> 验收标准:
> - [ ] 每个范式的每个核心指标都有对应的文献参考阈值
> - [ ] 分级结果附带文献来源引用
> - [ ] 多指标综合评估时给出综合判断和置信度
...
> - [ ] 自动计算衍生指标（比例值）
> - [ ] 基于 Rodgers et al. (1997) 等文献的阈值进行焦虑分级
...
> - [ ] 基于 Prut & Belzung (2003) 等文献的阈值进行焦虑分级
...
> **US-4.5**: 作为研究人员，我希望系统能分析新奇抑制摄食 (NSF) 数据，计算摄食潜伏期和接近次数，并进行组间比较。
>
> 验收标准:
> - [ ] 支持识别标准 NSF 数据列名（feedingLatency, approachCount, foodConsumption 等）
> - [ ] 摄食潜伏期作为核心焦虑指标
> - [ ] 基于文献阈值的焦虑分级
> - [ ] 结合家庭笼摄食量数据排除食欲因素干扰（如数据可用）

> **US-4.6**: 作为研究人员，我希望系统能分析悬尾实验 (TST) 数据，计算累计不动时间和不动潜伏期，并进行组间比较。
>
> 验收标准:
> - [ ] 支持识别标准 TST 数据列名（immobilityTime, immobilityLatency 等）
> - [ ] 累计不动时间作为核心抑郁样行为指标
> - [ ] 基于文献阈值的抑郁分级（不动时间 >150s 为高度抑郁样）
> - [ ] 不动潜伏期作为辅助指标（<20s 提示快速放弃行为）

> **US-4.7**: 作为研究人员，我希望系统能分析强迫游泳 (FST) 数据，计算累计不动时间和不动潜伏期，并进行组间比较。
>
> 验收标准:
> - [ ] 支持识别标准 FST 数据列名（immobilityTime, immobilityLatency 等）
> - [ ] 累计不动时间作为核心抑郁样行为指标（注意与 TST 的阈值差异）
> - [ ] 基于文献阈值的抑郁分级（不动时间 >200s 为高度抑郁样）
> - [ ] 区分不动、游泳、挣扎三种行为状态（如数据包含细分）

> **US-4.8**: 作为研究人员，我希望系统能分析糖水偏好 (SPT) 数据，计算糖水偏好率和液体总消耗量，并进行组间比较。
>
> 验收标准:
> - [ ] 支持识别标准 SPT 数据列名（sucroseConsumption, waterConsumption, totalIntake 等）
> - [ ] 自动计算糖水偏好率（糖水消耗/总液体消耗×100%）
> - [ ] 基于文献阈值的抑郁分级（偏好率 <65% 提示快感缺乏）
> - [ ] 液体总消耗量过低时排除渴觉因素干扰
...
| 知识库阈值不准确 | 中 | 高 | 基于已发表文献编码；支持用户反馈纠正机制 |
...
| 知识库 | YAML/JSON 结构化数据 | 范式-指标-阈值-文献映射 |
| 代码执行 | Docker 沙箱 | 安全隔离 |
| 数据存储 | 待定 | 用户数据和分析结果存储 |
...
> 各指标的参考范围和文献来源将在知识库构建阶段完成。本附录仅定义指标的计算方式和解读规则。
...
| 指标 | 英文名 | 计算方式 | 指标方向解读 | 运动混杂检查 |
|------|--------|---------|-------------|-------------|
| 摄食潜伏期 | feedingLatency | 首次开始进食的时间 (s) | 数值↑ 与焦虑样行为正相关 | 需排除体重/食欲基线差异 |
| 接近次数 | approachCount | 接近食物但未进食的次数 | 反映趋避冲突程度 | — |
...
| 指标 | 英文名 | 计算方式 | 指标方向解读 | 备注 |
|------|--------|---------|-------------|------|
| 糖水偏好率 | sucrosePreference | 糖水消耗/总液体消耗×100% | 数值↓ 与快感缺乏正相关 | 液体总消耗 <3mL 时排除渴觉因素 |
| 液体总消耗 | totalIntake | 糖水+清水总消耗 (mL) | 反映总体饮水动机 | — |
