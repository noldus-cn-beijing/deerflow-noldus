...
| 步骤 | MVP（R1 必须） | R2（扩展） | R3+（未来） |
|------|---------------|-----------|------------|
| **2.1 范式识别** | 基于列名自动识别 8 种范式 | 模糊匹配 + 置信度评分 | 自定义范式模板 |
...
**MVP 任务清单：**
- [ ] 基于列名模式匹配自动识别 8 种范式之一
- [ ] 识别结果卡片展示："检测到：高架十字迷宫 (EPM)"
- [ ] 识别失败 → 手动选择范式（下拉列表 8 选 1）
- [ ] 变量映射预览表（原始列名 ↔ 标准指标名）
- [ ] 缺失值检测：标记缺失率 > 20% 的变量
...
| **4.5 阅读指标解读** | 基于文献阈值的分级 + 文献引用 | 指标趋势跨实验对比 | 品系特异性阈值库 |
...
- [ ] 指标解读卡片：基于文献阈值的焦虑/抑郁分级（正常/轻度/中度/重度）+ 文献来源
...
**目标**：8 种范式完整分析流水线，全自动 + 手动两种模式

```
| 活动 | MVP 包含内容 |
|------|-------------|
| 导入数据 | CSV/Excel 上传、格式验证、进度反馈 |
| 确认解析 | 范式自动识别（8 种）、变量映射、质量检查 |
...
| **NSF** 新奇抑制摄食 | 摄食潜伏期、接近次数 | ✓ | 排除体重/食欲差异 | P0 |
| **TST** 悬尾实验 | 累计不动时间、不动潜伏期 | ✓ | 排除运动能力差异 | P0 |
| **FST** 强迫游泳 | 累计不动时间、不动潜伏期 | ✓ | 排除游泳能力差异 | P0 |
| **SPT** 糖水偏好 | 糖水偏好率、液体总消耗量 | ✓ | 总消耗 < 3mL → 警告 | P0 |
