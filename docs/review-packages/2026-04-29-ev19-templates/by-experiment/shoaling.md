# 实验：斑马鱼鱼群行为 (Shoaling (Zebrafish Group Behavior))

**slug**：`shoaling` （这是 paradigm key，agent 内部用）

> 行为学同事补充。机器侧 ev19_template 字段在 by-template/ 里维护。

<!-- 起草备注（同事可删除）：多鱼同竞技场，需要 IID/NND/polarity 指标。已有 ethoinsight/templates/shoaling.py -->

## 🟡 一句话定义（待补充）

<!-- 例：「测试啮齿动物在新环境中的探索-焦虑权衡」 -->

## 🟡 适用模板（按推荐顺序 + 取舍说明）

候选模板（起草时根据目录名猜的，**同事必须 review 删/改/补**）：

- `OpenFieldCircle-NoZones-Fish` —— 取舍：（待补充）
- `AquariumTrack3D` —— 取舍：（待补充）
- `DanioVision DVOC 004x-96w-circ` —— 取舍：（待补充）

<!-- 同事可自由增删。理想格式：
- `OpenFieldCircle-NoZones-Fish` — 推荐 / 优势 / 局限
- `AquariumTrack3D` — 何时用，何时别用
-->

## 🟡 必须计算的指标（待补充）

<!-- 例（shoaling）：mean IID（鱼间距）、mean NND（最近邻距）、polarity（队列度） -->

- 

## 🟡 常见脱险点 / 数据质量风险（待补充）

<!-- 例：「样本量小于 n=8 / 组时统计功效不足」「视频抖动会污染 IID 计算」 -->

- 

## 🟡 报告解读语言（待补充）

<!-- 用什么名词、什么公式表达、避免什么常见误读 -->

## 🟡 关键参考文献（待补充）

- 

## 🟡 与其他实验的区分（待补充）

<!-- 例（shoaling vs aquatic_open_field）：「都用 OpenFieldCircle-NoZones-Fish
但 shoaling 是多鱼同时记录测群体度，aquatic_open_field 是单鱼测探索-中心回避」 -->
