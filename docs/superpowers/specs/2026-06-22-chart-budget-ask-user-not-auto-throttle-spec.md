...
> ⚠️ **实施裁决（2026-06-22 用户裁决， supersede §〇/§2.2/§2.3/§三/§四/§五 的「阈值反问」设计）**：
> 实施时用户进一步裁定「**画图部分不该限制资源，有多少画多少**」。因此本 spec 原设计的
> 「subject 数 > 阈值时 lead 主动反问全画/子集」**未实施**——主动反问会把「画多少」这个本不该
> 用户操心的决策抛回给用户。**实际实施 = 默认全画**：用户说要图就全画（`chart_budget=None`，
> 不限资源），lead 不主动反问全画/子集；**仅当用户原话主动表达「画几张就行/代表性/少画点/
> 挑几个」时**，lead 才给定 `chart_budget` 数字并透传给 chart-maker。chart-maker 绝不自行设预算。
> ASKVIZ「要不要图」A/B 反问保留（用户：先问画不画）。下方原 spec 正文保留作设计沿革记录。

## 〇、给实施 agent 的一句话（原设计，已被上方裁决 supersede）
