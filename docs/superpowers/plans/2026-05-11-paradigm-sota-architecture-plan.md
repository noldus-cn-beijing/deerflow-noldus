...
| `ethoinsight-analysis` | **重命名为 `ethoinsight-code` + 大改** | 目录 mv；删 5 工具流程描述；改写为「按 paradigm 渐进披露 + 胶水脚本范例 + handoff schema」 |
...
skills=["ethoinsight-analysis"]   →   skills=["ethoinsight-code", "ethoinsight-charts"]
...
**目标:** 用 EPM 当试金石跑通整套 SOTA 流程。完成时：metrics.py 已拆为 metrics/ 子模块、ethoinsight-analysis 已改名为 ethoinsight-code、code-executor 已切换白名单、demo EPM 数据用胶水脚本能跑出 handoff JSON + 至少 1 张 PNG 图。
...
### Task 3: 重命名 skill 目录 ethoinsight-analysis → ethoinsight-code

**Files:**
- Move: `packages/agent/skills/custom/ethoinsight-analysis/` → `packages/agent/skills/custom/ethoinsight-code/`
- Modify: `packages/agent/backend/packages/harness/deerflow/subagents/builtins/code_executor.py:91` — `skills=["ethoinsight-analysis"]` → `skills=["ethoinsight-code", "ethoinsight-charts"]`
...
```bash
cd /home/wangqiuyang/noldus-insight
grep -rn "ethoinsight-analysis" --include="*.py" --include="*.md" --include="*.json" --include="*.yaml" \
...
```bash
cd /home/wangqiuyang/noldus-insight
git mv packages/agent/skills/custom/ethoinsight-analysis \
...
- [ ] **Step 3.4: 把 Step 3.1 grep 结果里其它所有 `ethoinsight-analysis` 引用全改为 `ethoinsight-code`**

逐个文件 sed 或手动改。重点检查的位置：
- `packages/agent/extensions_config.json`（如果有）
- `docs/` 下任何提到 `ethoinsight-analysis` 的文档
...
cd /home/wangqiuyang/noldus-insight
git add -A
git commit -m "refactor: skill ethoinsight-analysis → ethoinsight-code + code-executor 白名单加 ethoinsight-charts（Phase 1 T3）"
```
...
- [ ] skill 目录已重命名 `ethoinsight-analysis` → `ethoinsight-code`，所有引用同步
...
| skill 文件 mv 后 import 仍找 `ethoinsight-analysis` | 漏 grep 漏改的引用 | `grep -rln "ethoinsight-analysis" packages/` | 全文替换为 `ethoinsight-code` |
