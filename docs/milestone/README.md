...
|---------|------|-----------|-------------|
| **前端生成式 UX Phase 1（输出理念重构）** | 📋 路线图已立项（北极星=Agent 驱动生成式 UI；分步骤 A1 后端事件分轨地基→A2 前端分轨渲染→C1-3 画廊一等公民/框选追问；infra 全用 DeerFlow 现成，翻译层=后端中间件由真实事件驱动不靠 LLM 自报，不引入 ag-ui 仅借分轨语汇）· **A1 已派实施**；dogfood 3 显示 bug 全修合 dev（dots #247 / subtask 卡运行态 #248 / 崩溃重连空转+409 #249）· 切回卡顿 prod 实测不复现（dev build 假象，续 spec 已删）· dark mode/replay 仍待排期 | [2026-06-30 路线图+A1 设计](../superpowers/specs/2026-06-30-generative-ux-roadmap-and-a1-event-track-foundation-design.md) | [6/30 darkmode+dogfood 三 bug spec](../handoffs/2026-06/2026-06-30-frontend-darkmode-and-dogfood-three-bug-specs-handoff.md) |
