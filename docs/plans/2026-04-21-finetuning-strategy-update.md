...
>
> 我们的范围：v0.1 要覆盖 `demo-data/DemoData/` 下的**全部 16 个行为学范式**的深度分析能力（高架十字迷宫、旷场、强迫游泳、莫里斯水迷宫、Y/O 迷宫、巴恩斯、新物体识别、三箱社交、社会互动、明暗箱、新奇抑制摄食、足迹、精细行为、PhenoTyper、斑马鱼鱼群）。好消息是 agent 框架已经跑通（通过斑马鱼验证），新增范式的工程工作很轻——大部分时间花在**你的专业标注**上，让 agent 对齐专家判断。
>
> 我需要你帮忙做三件事：
>
> **事 1（本周）**：过一遍我发给你的 `case-001-shoaling-baseline`。看看 `expected-analysis.yaml` 是不是能表达你做分析时的"期望结论"？有没有字段缺失？欢迎直接在文件里改。这份结构会作为后续 15 个范式标注的模板。
>
> **事 2（5-8 月，分批标注全部 16 个范式）**：每个范式提供 1 份**深度 golden-case**，包含真实实验数据（5-10 个受试者的 EthoVision 导出）+ 你预期的分析结论（什么正常、什么异常、什么混杂因素）。按优先级分三批：
>   - 5 月：EPM / 旷场 / 强迫游泳（焦虑抑郁核心）
>   - 6 月：MWM / Y迷宫 / O迷宫 / 巴恩斯 / 新物体识别（空间记忆 + 识别记忆）
>   - 7-8 月：三箱社交 / 社会互动 / 明暗箱 / 新奇抑制摄食 / 足迹 / 精细行为 / PhenoTyper（其他）
>
>   每份标注约 3-5 小时。如果某些范式的数据你手头就是现成的（做过的实验），优先用现成数据。
>
> **事 3（5-6 月起持续）**：review 我们写的"异常诊断模式库"（文档 §8 四种经典异常）。看看分类对不对、区分标志在实战里用不用得上、有没有漏的常见异常类型。随着新范式覆盖会陆续发现范式特定的异常模式。
>
> **总投入估计**：标注 16 个 golden-case（80-100 小时，分摊 4 个月） + rubric 抽检和 review 工作（20-30 小时）。**如果你一个人扛压力大，我们可以再拉 1-2 位同事分担**——关键是每份标注的质量，而不是谁做的。
>
...
### 3.2 Step 2（5 月，4 周）：第一批范式批量补全 + 基建抽象 + 行为学启动

**关键认知**：shoaling 已经把 agent 框架跑通。每个新范式的**真实边际成本是 3-5 天**（填模板 + 补几个指标函数 + 填阈值 YAML）。所以本 Step 不再是"做 1 个 EPM"，而是：

**工程线**：

1. **基建抽象**（跨范式一次性投入，1.5 周）
   - 抽 `ethoinsight/templates/_base.py`：把 shoaling.py 的参数块 + 6 步工作流 + handoff 写入骨架抽成可复用基类
   - 重构 shoaling.py 改为调用基类（验证抽象正确）
   - 把 `metrics.py` 的指标分族：空间位置类 / 运动学类 / 社交类 / 时间序列类——新指标按族实现，多范式复用
   - 统一 `assess.py` 阈值 YAML schema（分物种/分时长分层）
2. **第一批范式批量补全**（2 周，可并行）
   - EPM（高架十字迷宫）— 开/闭臂 + 中心时间指标
   - Open Field（旷场实验）— 中心/外周时间、运动距离
   - FST（强迫游泳）— 不动时间识别（速度阈值）
   - 每个范式交付 §7.2 八件套，但 **golden-case 的深度标注暂缓到行为学同事推进节奏**
3. **MWM（莫里斯水迷宫）启动**（1 周）— 学习曲线 + 象限分析，比前 3 个复杂，可以跨到 Step 3

**行为学线**：
- Step 1 的 case-001 样板上手，标注第一批的 golden-case：**EPM / OFT 各 1 份深度标注**
...
**这一步完成的标志**：
- [ ] `templates/_base.py` 抽象完成、shoaling.py 重构为调用基类
- [ ] EPM / OFT / FST 三个范式 E2E 跑通 demo
- [ ] MWM 已启动（八件套 ≥ 50% 完成）
- [ ] case-002（EPM） / case-003（OFT）深度标注完成
- [ ] §8 异常模式库 v0.2（融入同事第一轮反馈）

### 3.3 Step 3（6 月，4 周）：第二批范式批量补全 + Golden-case 自动化 + quality-reviewer 原型

**工程线**：

1. **Golden-case 自动化**（0.5 周）
   - `tests/test_golden_cases.py` runner：读 `expected-analysis.yaml` → 跑 E2E → 断言
   - `make test-golden` 命令
   - case-001/002/003 跑通第一次自动化回归
2. **MWM 收尾** + **第二批范式补全**（2 周，可并行）
   - MWM（莫里斯水迷宫）收尾 — 上 Step 启动
   - Y 迷宫 — 自发交替、工作记忆指标
   - O 迷宫 — 与 EPM 共享指标（§7.6 已预埋 `o_maze` 阈值）
   - 巴恩斯迷宫 — 与 MWM 共享部分指标
   - 新物体识别 — 探索偏好指数
3. **quality-reviewer 原型实装**（降级路径版，§6.4）（1.5 周）
   - 新 subagent 注册 `quality_reviewer.py`
   - ReviewFinding schema 落代码
   - 集成到 lead agent 的场景参考表（noldus-kb 仍禁用，走降级路径）
   - 在 shoaling/EPM/OFT 上跑互审验证

**行为学线**：
- 深度标注第二批范式 golden-case：**MWM / Y迷宫 / 新物体识别 各 1 份**（3 份）
- 选 1 个典型 shoaling/EPM 分析，手动写"理想初版洞察 + 期望 reviewer 反馈"——DPO 种子
- 补充标注 §8 异常模式库的**范式特定异常**（比如 MWM 的"无策略游泳"、Y迷宫的"偏侧化"）

**这一步完成的标志**：
- [ ] Golden-case 自动化 runner 可用，每日回归
- [ ] 第一批 + 第二批共 8 个范式（+ shoaling 共 9 个）八件套齐全
- [ ] 8 份深度 golden-case 标注完成
- [ ] quality-reviewer 降级版上线，在 3 个范式上 E2E 验证
- [ ] 2 份专家手写"初版 vs 理想版"作为 DPO 种子

### 3.4 Step 4（7 月，4 周）：第三批范式补全 + SFT 数据采集 + 训练（roadmap M1.1-M1.2）

**工程线**：

1. **第三批范式补全**（2 周，可并行）
   - 三箱社交测试 / 社会互动测试 — 社交范式指标（接触、互动事件）
   - 明暗箱 — 区域时间 + 跨越事件
   - 新奇抑制摄食 — 潜伏期测量
   - 足迹分析 — 步态指标（范式特有）
   - 动物精细行为识别 — 事件识别类数据，可能需要 parser 扩展
   - PhenoTyper 硬件系统 — 长时程监测，可能需要采样降频
2. **SFT 数据采集**（与范式并行，1.5 周）
   - `scripts/extract_e2e_sessions.py`：从 `.deer-flow/threads/` 和导出 markdown 抽 (input, thinking, output)
   - `scripts/generate_synthetic_data.py`：用 claude-sonnet-4-6 按场景参考表 A-E × 覆盖全部 16 范式生成 400+ 条变体数据
   - `scripts/generate_stats_qa.py` + `scripts/generate_skill_qa.py`：代码和 skill reference 自动转 QA
3. **数据筛选与格式转换**（0.5 周）
   - 合成数据交行为学同事抽样审核（10%）
   - 合格率 < 80% → 调整生成 prompt 重跑
   - `scripts/convert_to_fireworks_jsonl.py` 产出带 `<think>` 的 ChatML
4. **上传 Fireworks 训练**（0.5 周）
   - Qwen3-8B + LoRA rank 16 + 默认超参 → SFT checkpoint
   - Golden-case 回归：跑所有已有 case，和 claude-sonnet baseline 对比

**行为学线**：
- 合成数据抽样审核（约 5-10 小时）
- 关键场景手写（50-100 条）：场景参考表 A-E 最难的、异常模式 A-D 典型解读、reviewer 批判示范
- 深度标注第三批 golden-case（目标 5-6 份，覆盖批次里最重要范式）

**这一步完成的标志**：
- [ ] 全部 16 个范式八件套齐全
- [ ] SFT 训练数据集 ~800+ 条，带 `<think>`、合格率 > 80%
- [ ] Qwen3-8B SFT checkpoint 可跑 inference
- [ ] Golden-case 对比：SFT 模型 ≥ claude-sonnet baseline
- [ ] 累计 13-14 份深度 golden-case（覆盖大部分范式）

### 3.5 Step 5（8 月，4 周）：模型切换验证 + golden-case 收齐 + v0.1 收尾

**工程线**：
- **渐进模型切换**（roadmap M1.3）：先换 subagent 模型（data-analyst / code-executor），最后换 lead——每一步跑 golden-case
- Rubric 抽检（§9.7）：行为学同事打分
- 查漏补缺：范式填充细节、prompt 微调、边界问题修复
- v0.1 上线前 checklist（配置、部署、文档）

**行为学线**：
- Rubric 打分（每范式 1-2 小时 × 16 = 20-30 小时，可分多位同事）
- 补齐剩余范式的深度 golden-case（目标：**16 范式 × 1 份深度 = 16 份**）
- 为 v0.1 发布写"使用指南"或"测试报告"（可选）

**这一步完成的标志**：
- [ ] 16 范式 × 16 份深度 golden-case 全绿
...
设计这份计划时，有意识地把同事的投入**从浅到深**排列，让他们的时间价值越来越高。因为 v0.1 扩展到 16 范式覆盖，总投入也相应增加——**建议拉 2-3 位同事分担**：

| 阶段 | 工作类型 | 门槛 | 时间投入 |
|---|---|---|---|
| Step 1 | 给一份 YAML 样板提建议 + 启动会 | 低（看看填什么） | 2-3 小时 |
| Step 2 | 标注第一批 3 份 golden-case（EPM/OFT/FST） | 中（按 YAML 填） | 12-15 小时 |
| Step 3 | 标注第二批 5 份 golden-case（MWM/Y迷宫/O迷宫/巴恩斯/新物体识别）+ DPO 种子对 | 中偏高 | 20-25 小时 |
| Step 4 | 审核合成数据 + 关键场景手写 + 第三批 5-6 份 golden-case | 高（最专业投入） | 25-35 小时 |
| Step 5 | Rubric 评分 + 补齐剩余 golden-case 至 16 份 | 中（系统性评估） | 20-30 小时 |

**总计 80-110 小时，分摊到 4 个月**——单人每周 5-7 小时 **or** 2-3 位同事每人每周 2-3 小时。后者更可持续。

**关键节奏**：如果只有 1 位同事，golden-case 进度可能成为瓶颈。考虑：
- 同一范式由熟悉该范式的同事标注（提高效率）
- 两批之间允许并行（同事 A 做 Step 3 标注时，同事 B 已经开始 Step 4 的审核）
- 优先保证**前 8 个范式**（第一批 + 第二批）的深度 golden-case 质量，后 8 个范式标准可以在不牺牲深度的前提下流程化
...
| Golden-case 数据量不够 | 中 | 高 | 早期就用 fix5 的 shoaling 数据起步（Step 1），不等完美；16 范式分三批推进，前 8 个优先 |
| 16 范式覆盖节奏跟不上 | 中 | 中 | 框架级工作优先（一次性投入 5-8 周），范式填充可批量 + 并行，每范式边际成本 3-5 天 |
