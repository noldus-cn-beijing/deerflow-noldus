...
## 2026-05-29（下午）愿景架构对齐：声明式编排 vs 动态编排 + 两模式假说

> **本节性质**：这是一次长对话（编排哲学 + 产品愿景深挖）的结论沉淀，供下一会话/下一 agent 无缝接续。**其中"两模式架构"是待确认的假说，不是已锁定的结论**——决定性开关见 §D，必须先向行为学同事核实后才能据此排 sprint。
>
> 关联文档：[编排路径 SSOT 诊断设计](../superpowers/specs/2026-05-29-orchestration-path-ssot-diagnosis-design.md)（§7 收录了本对话的编排哲学论证细节）。

### A. 触发与收敛过程（防止下一 agent 重走认知弯路）

用户读了一篇讲 Claude Code Dynamic Workflows（运行时 LLM 即兴写 JS 编排脚本）的文章，问"我们要不要也上动态编排，那样才像真正的智能 agent"。经多轮 grill，收敛出一个**贯穿现在与将来的结论**：

> **EthoInsight 的全链路（实验设计 → 采集 → 分析）是「看重时间的、由 Noldus 自己定义边界的、必须可复现的有限集」。所以全链路都该是「声明式编排骨架 × 节点内 LLM 判断智能」，不需要 Claude Code 式的动态编排——现在不需要，将来更不需要。**

**三把判据（任一成立即排斥动态编排，三者叠加为铁证）**：

1. **可枚举 vs 不可枚举**：Claude Code 面对"编程/修任意 bug"——任务空间本质开放、事前画不出流程，**只能**运行时即兴。EthoInsight 面对成熟行为学范式——流程教科书级确定。**关键身份事实：我们是 Noldus，任务空间的边界是我们自己用产品定义的**（EV19 模板本就是有限枚举），不是去猜一个外部开放空间。
2. **看重时间，不容试错**：动态编排的本质是"即兴 + 允许回溯试错"。但实验科学要求"一次做对"——尤其将来接管**采集**（实时、不可逆、毫秒级时序），"试错"= 一只动物的实验作废。时间敏感性将来只增不减 → 对动态编排的排斥**更强**。
3. **可复现 = 科学仪器的命**：动态编排"每次可以不一样"与"同一输入必须可复现同一结果"直接对立。我们要成为科学仪器（论文级可重现），可复现是不可谈判的底线。

**"智能"放在哪（回答"那还算不算智能 agent"）**：智能**不在编排的自由度**，而在**节点内的判断质量**——看着 0.56s 不动时间能推断"mobility 参数未校准"，从"测抗焦虑 + 预算有限"能推出"做 OFT、n=10、功效够且省成本"。**节点间用声明式骨架保证鲁棒与可复现，节点内让 LLM 判断/解读/决策智能充分爆发。** 这两件在不同层，不冲突——这就是"最鲁棒"和"最智能"能同时成立的原因。

**一个必须钉死的区分（下一 agent 最易混淆点）**：**意图路由 ≠ 动态编排**。"用户来了不知道他想配模板/做实验/查记录/分析/问问题"——这是**意图路由**（在有限 INTENT 清单里选一条，LLM 判断 + 不确定走 CLARIFY 反问），不是动态编排（路径事前画不出）。能把这 5 个场景列出来，本身就证明它们可枚举。**场景从 8 涨到 800 只是 SSOT 规模变大，而规模越大越需要可枚举/可检查/可复现，越不能交给 LLM 即兴。**

### B. 愿景定义（用户原话）

> 现在的 agent 只专注分析 EV19 产出的数据。**最终愿景**：彻底接替 EV19，成为"让实验人员用自然语言搭建模板 → 进行实验数据采集 → 分析"的全能行为学 agent。

这是数量级更大的东西——从"分析工具"到"覆盖实验设计 + 采集 + 分析的全生命周期科研仪器"。**身份前提**：我们是 Noldus，对 EthoVision 的实验本体、数据格式、采集逻辑有完整定义权——这把"自然语言配实验"从"蹭外部闭源软件"变成"给自己的产品做 AI 层"。

### C. 愿景的三个深水区（真正的难点，不在编排范式，在这三件 GUI 做不到的事）

1. **设计层智能 ≠ 分析层智能**：分析层是**回溯判读**（数据已在，错了能重算）；设计层是**前瞻决策**（信息不全时下注，错了几周/几万块/几十只动物后才暴露，不可逆）。设计层 agent 必须具备分析层不需要的能力：power analysis 前瞻推理、样本量/预算/效应量的**权衡协商**、以及**拒绝的勇气**（用户说"用 3 只动物得显著结论"要能说"统计上做不到，这是为什么"）。**这是从"听话的分析工具"进化成"会说不的科研合作者"——这才是 AI 相对 GUI 的真正增量，不是"自然语言入口"这种表层。**
...
| EV19 模式 | 说明 | 任务空间 |
|---|---|---|
| **模板模式** | 选已有模板（EPM/OFT…）直接在设置好的定制环境跑 | 有限可枚举 |
| **自由模式** | 用户自定义各种实验情况和仪器（**但跑不出行为学范畴**） | 大但——见下 |

**用户的直觉**："是不是该准备两套编排，一个自由一个有限集？"

**修正后的架构论断（假说）**：**不是"两套编排引擎"，而是"一套声明式引擎 + 两种粒度的声明式构件"**：
- **模板模式** = 自由模式构件的**预组合快照**（粗粒度，如现在的 `PATHS[EPM]` 整条路径）
- **自由模式** = 行为学**有限基元**（追踪点 / zone 几何 / 事件谓词 / 计时器 / 阈值 / 分组）的**自由组合 + 组合合法性规则**（细粒度）
- 两者都是**声明式数据**，共用同一引擎、同一基元真相源、同一套可复现/可追溯机制
- LLM 智能落在"把自然语言意图翻译成「用哪些基元、怎么合法组合」"——**结果是确定性的声明式产物**

**为什么不做"两套引擎"**：自由模式那套若用动态 JS → ① 无法可复现（违背科学仪器底线）② 产物非声明式数据、分析层读不了（契约断裂）③ 双真相源（违背 SSOT）。
