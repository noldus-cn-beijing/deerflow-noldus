...
- **范围**: v0.1 的判断能力 — 16 个范式的标准分析 + 洞察审核机制 + 异常诊断模式库
...
1. 16 个范式（`demo-data/DemoData/` 下全部）能端到端分析，输出质量不低于 shoaling 当前水平
...
v0.1 9 月硬指标要求 16 个范式完整可用（`demo-data/DemoData/` 下全部）。shoaling 已经验证了通用 agent 框架，剩下 15 个范式的增量工作是**每范式 3-5 天的填空**。关键不是"每个范式从零设计"，而是**把 shoaling 抽象成标准模板**，新范式只填空、不重新发明。
...
**重要前提认知**（2026-04-22 更新）：shoaling 的工作**不是"做一个范式"，是做出一套可以跑通任何范式的通用 agent 框架**。具体讲 pipeline（code-executor → data-analyst → report-writer）、handoff JSON 契约、SubtaskCard 可见性、语言一致性、洞察结构、parse/statistics/charts 的基础能力——这些全部已经在 shoaling 上验证过、跨范式通用。

因此新增范式的**真实增量工作**小得多：
- `templates/<paradigm>.py`（填参数、指标列表、图表选择）— 纯填空
- `metrics.py` 补范式特定指标 — 多数跨范式共享（距离/速度/区域时间），只有范式特有指标需要新写
- `assess.py` 补阈值 — 纯 YAML
- skill reference planning 提示 — 纯 Markdown
- Demo 数据 + golden-case — 已有 demo-data、需要行为学同事标注

**每个新增范式的边际成本：3-5 天**，不是原估计的 1-2 周。

v0.1 覆盖 **全部 16 个范式**（`demo-data/DemoData/` 下全部），按 Noldus 用户使用频率和文献成熟度排优先级：

**第一批：核心焦虑/抑郁/空间记忆范式**（5-6 月，已是当前 roadmap 计划）

| 顺序 | 范式 | 类型 | 重点考虑 |
|---|---|---|---|
| 1（已完成） | 斑马鱼鱼群行为（shoaling） | 群体行为 | 框架打磨完成 |
| 2 | 高架十字迷宫（EPM） | 焦虑 | 最经典、文献阈值最明确 |
| 3 | 旷场实验（Open Field） | 焦虑 + 运动 | 指标简单、跨范式指标集中 |
| 4 | 强迫游泳实验（FST） | 抑郁 | 不动时间识别需细调速度阈值 |
| 5 | 莫里斯水迷宫（MWM） | 空间学习记忆 | 学习曲线、象限分析 |

**第二批：常用迷宫 + 社交 + 识别范式**（7 月）

| 顺序 | 范式 | 类型 | 重点考虑 |
|---|---|---|---|
| 6 | Y 迷宫 | 工作记忆 | 自发交替，指标计算直接 |
| 7 | O 迷宫 | 焦虑 | 与 EPM 极相似（已预埋 `o_maze` 阈值） |
| 8 | 巴恩斯迷宫 | 空间记忆 | 与 MWM 共享部分指标 |
| 9 | 新物体识别 | 识别记忆 | 探索偏好指数 |
| 10 | 三箱社交测试 | 社交 | 区域时间 + 偏好计算 |
| 11 | 社会互动测试 | 社交 | 互动事件识别 |

**第三批：特殊范式 + 冷门/新兴范式**（8 月）

| 顺序 | 范式 | 类型 | 重点考虑 |
|---|---|---|---|
| 12 | 明暗箱 | 焦虑 | 区域时间 + 跨越事件 |
| 13 | 新奇抑制摄食 | 焦虑 + 动机 | 潜伏期测量 |
| 14 | 足迹分析 | 运动学 | 步态指标（范式特有） |
| 15 | 动物精细行为识别 | 多维行为 | 事件识别类数据，指标形态不同 |
| 16 | PhenoTyper 硬件系统 | 长时程监测 | 硬件特异，可能需要 parser 扩展 |

### 7.6a 工程节奏

**一次性投入**（v0.1 真正的核心工作，跨范式通用）：
- §5 Lead agent 能力菜单扩展（场景参考表 A-E + ask_clarification 一等公民化）
- §6 Quality-reviewer subagent 完整设计与实装
- §8 异常诊断模式库（A/B/C/D 四类 + 检测工具）
- §9 Golden-case 自动化 runner + expected-analysis.yaml schema
- `templates/_base.py` 基类抽象（shoaling.py 重构入口）
- 预计 5-8 周

**增量投入**（每个新范式 3-5 天，可并行、可批量）：
- 按上面三批优先级推进
- 框架级工作稳定后，多个范式可以并行填充
- 每个范式交付"八件套"（§7.2）

**总节奏估计**：
- 5 月底：第一批前 3 个完成（EPM/OFT 合并 FST 启动）
- 7 月底：第一批 + 第二批全部完成（共 11 个范式）
- 8 月底：全部 16 个范式八件套齐全 + SFT 微调（Phase 1 M1.2）完成
- 9 月初：v0.1 交付
...
一次性收集 16 个完整 case 不现实。建议分批递进（对应 finetuning §3 的 5 个 Step）：

| 阶段 | 目标 | 来源 | 时间 |
|---|---|---|---|
| **Step 1（现在）** | 1 个 shoaling 基线 case | 用 fix4/fix5 已跑过的 thread 数据（6f046cc7...）+ 补 expected-analysis.yaml | 1 周内 |
| **Step 2（5 月）** | +3 个（EPM / OFT / FST） | 行为学同事标注第一批（焦虑/抑郁核心） | 与第一批范式实装同步 |
| **Step 3（6 月）** | +5 个（MWM / Y迷宫 / O迷宫 / 巴恩斯 / 新物体识别） | 行为学同事标注第二批（空间/识别记忆） | 与第二批范式实装同步 |
| **Step 4（7 月）** | +5-6 个（三箱社交 / 社会互动 / 明暗箱 / 新奇抑制 / 足迹 / 精细行为） | 行为学同事标注第三批 | 与第三批范式实装同步 |
| **Step 5（8 月）** | 补齐至 16 个（PhenoTyper + 前面任何漏的） | 行为学同事收尾 + Rubric 抽检 | v0.1 交付前 |
| **v0.1+** | 扩到 32+ case，含边缘场景（小样本、缺失数据、设备故障数据） | 持续收集 | 持续 |

每个 case 的前置成本：行为学同事 3-5 小时标注 + 工程师 1 小时转 YAML + 写测试。**16 范式总投入约 80-100 小时行为学标注，建议多位同事分担**。
...
> **关于范式覆盖面**（2026-04-22 更新）：v0.1 覆盖 `demo-data/DemoData/` 下**全部 16 个范式**的深度分析能力，不再是原定的 5 个。理由：shoaling 的工作已经验证了通用 agent 框架，新增范式的边际成本（3-5 天/范式）远小于原估计。详见 §7.6 和 [finetuning-strategy-update.md §3](2026-04-21-finetuning-strategy-update.md)。
