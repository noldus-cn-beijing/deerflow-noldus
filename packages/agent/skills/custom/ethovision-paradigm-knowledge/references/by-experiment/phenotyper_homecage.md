# 实验：PhenoTyper 居家观察 (PhenoTyper Home-Cage Observation)

**slug**：`phenotyper_homecage` （这是 paradigm key，agent 内部用）

> 行为学同事补充。机器侧 ev19_template 字段在 by-template/ 里维护。

<!-- 起草备注（同事可删除）：长期居家行为，不是短时 trial。有大量阵列变体 -->
...
- `PhenoTyper-AllZonesMice` —— 取舍：（待补充）
- `PhenoTyper-AllZonesRatOther` —— 取舍：（待补充）
- `PhenoTyper-FeedingShelterMice` —— 取舍：（待补充）
- `PhenoTyper-NoZones` —— 取舍：（待补充）
- `PhenoTyper-Quad-AllZonesMice` —— 取舍：（待补充）
- `PhenoTyper-16x-AllZonesMice` —— 取舍：（待补充）
