...
| **Artifacts** (`/api/threads/{id}/artifacts`) | `GET /{path}` - serve artifacts; active content types (`text/html`, `application/xhtml+xml`, `image/svg+xml`) are always forced as download attachments to reduce XSS risk; `?download=true` still forces download for other file types; `GET /report/export?format=pdf|docx|tex` - convert the thread's `report.html` to PDF (WeasyPrint) / docx / LaTeX (pandoc) and return as attachment (LaTeX path pre-extracts base64 data-uris; spec 2026-06-29-report-export-formats-impl) |
| **Suggestions** (`/api/threads/{id}/suggestions`) | `POST /` - generate follow-up questions; rich list/block model content is normalized before JSON parsing |
| **Feedback** (`/api/threads/{tid}/runs/{rid}/feedback`) | `POST /` - submit verdict-based feedback (correct/needs_fix/wrong + optional revised_text); `GET /` - list current user's feedback for a run. Backed by FeedbackRepository (SQLite). Requires `@require_permission("threads", "write"/"read")` |
