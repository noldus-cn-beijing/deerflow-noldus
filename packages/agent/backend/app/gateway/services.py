...
def build_run_config(
    ...
    *,
    ...
    # Honour an explicit agent_name in either runtime options container, and
    # dual-write the resolved name into both ``configurable`` and ``context`` so
    # it is visible to legacy configurable readers and to LangGraph
    # ``ToolRuntime.context`` consumers (e.g. the ``setup_agent`` tool, which
    # since LangGraph >=1.1.9 no longer falls back from ``context`` to
    # ``configurable``).
...
async def start_run(
    ...
    # thread access. Legacy IM channel calls that do not yet carry the owner
    # header keep the historical bypass (internal system role is trusted).
    user = getattr(request.state, "user", None)
    if user is not None and not (getattr(user, "system_role", None) == INTERNAL_SYSTEM_ROLE and not owner_user_id):
