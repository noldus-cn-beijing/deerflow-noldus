...
def format_memory_for_injection(
    ...
    **2026-06-24 跟随上游 #3592**：在隔离基础上叠加 guaranteed injection
    —— 某些 category 的 fact（如 correction）即使常规 token budget 耗尽
    也保证注入，带 graceful fallback + structure-aware truncation（Facts
    块作为 protected suffix 不被 prefix-cut 丢弃）。与隔离正交：隔离决定
    *注入哪些字段*，guaranteed 决定 *facts 内部哪些优先保全*。
    """
    if not memory_data:
        return ""

    # Pre-initialise facts-block markers so the structure-aware truncation
    # below can reference them even when no facts survive (avoids NameError
    # on the narrow "no facts + user-context overflow" path).
    facts_header = "Facts:\n"
    ...
    # Noldus 2026-05-13 定制：砍掉上游的 topOfMind + history.* 段（见 docstring）。
    ...
    # so they cannot be evicted by regular facts.  Regular facts draw from
    # *max_tokens* only.  When the primary path raises any exception,
    # ``_fallback_format_facts`` performs a single-pass confidence-only ranking.
    facts_data = memory_data.get("facts", [])
    guaranteed_line_tokens = 0  # used later for the effective truncation limit
    if isinstance(facts_data, list) and facts_data:
        # Token cost of sections built above (user context).
        base_text = "\n\n".join(sections)
