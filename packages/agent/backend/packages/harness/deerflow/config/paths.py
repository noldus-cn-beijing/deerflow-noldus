...
class Paths:
    ...
    def shared_dir(self, thread_id: str, *, user_id: str | None = None) -> Path:
        """
        Host path for the shared workspace between lead agent and subagents.
        Host: `{base_dir}/threads/{thread_id}/shared/`
        Sandbox: `/mnt/shared/`
        """
        return self.thread_dir(thread_id, user_id=user_id) / "shared"
    ...
    def ensure_thread_dirs(self, thread_id: str, *, user_id: str | None = None) -> None:
        ...
        for d in [
            ...
            self.shared_dir(thread_id, user_id=user_id),
        ]:
