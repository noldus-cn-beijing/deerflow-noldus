...
2. for entry in plan.metrics:
     ...
     **批量并行执行**：对同指标类型的不同 subject（多条 entry 共享同一个 script），用一条 bash 并行：
     ```
     bash -c "
     python -m ethoinsight.scripts.epm.compute_open_arm_time_ratio --input /mnt/user-data/uploads/s1.xlsx --output /mnt/user-data/workspace/m_open_arm_time_ratio_s0.json --parameters-json '{"open_arm_zones":["open"]}' &
     python -m ethoinsight.scripts.epm.compute_open_arm_time_ratio --input /mnt/user-data/uploads/s2.xlsx --output /mnt/user-data/workspace/m_open_arm_time_ratio_s1.json --parameters-json '{"open_arm_zones":["open"]}' &
     wait
     echo 'ALL_DONE_open_arm_time_ratio'
     "
     ```
     效果：140 个 individual bash 调 → ~5-8 个批量 bash（按指标类型数），节省 turn 预算。
