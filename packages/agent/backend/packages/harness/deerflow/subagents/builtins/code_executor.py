...
3. for entry in plan.metrics:
     ...
     每个脚本 stdout 末尾会有 `[result] {json}` 行，抓出来留作聚合用。每个 [result] JSON 含 parameters_used 字段（可能为空 dict）。
...
5. 聚合：把所有 metrics[].output 的 JSON 内容（含 parameters_used）+ statistics 输出（如有）合并构造 handoff。
   每个结果 JSON 中的 parameters_used dict 透传到 metrics_summary 对应条目的 parameters_used 字段。
