...
2. read `/mnt/user-data/workspace/plan_metrics.json` — 这是 lead 已经生成好的施工单，含 paradigm、metrics[]、statistics、skipped[]、inputs.groups_file（可能为 null）
...
5. 聚合：把所有 metrics[].output 的 JSON 内容 + statistics 输出（如有）合并构造 handoff
...
- **不存在的脚本立即停手**：bash 报 `No module named` / return code 非零后，**不要重试不同名字**(详见 ethoinsight-grouping skill §不存在脚本应对)。
