...
1. read `/mnt/user-data/workspace/plan_metrics.json` — 这是 lead 已经生成好的施工单，含 paradigm、metrics[]（每条含 script/input/output/args）、statistics、skipped[]、inputs.groups_file（可能为 null）
2. for entry in plan.metrics:
     ...
     **批量并行执行**：对同指标类型的不同 subject（多条 entry 共享同一个 script），用一条 bash 并行：
     bash -c '
     python -m ethoinsight.scripts.epm.compute_open_arm_time_ratio --input /mnt/user-data/uploads/s1.xlsx --output /mnt/user-data/workspace/m_open_arm_time_ratio_s0.json &
     python -m ethoinsight.scripts.epm.compute_open_arm_time_ratio --input /mnt/user-data/uploads/s2.xlsx --output /mnt/user-data/workspace/m_open_arm_time_ratio_s1.json &
     wait
     echo ALL_DONE_open_arm_time_ratio
     '
     ```
     entry.args 已含 --parameters-json（如有），直接从 entry.args 逐条照抄即可，不要在示例里硬编码参数 JSON。
     ...
     **VALIDATION_ERROR 行抓取（S2 metric validator 产出）**：每个脚本 stdout 可能包含 `VALIDATION_ERROR: <metric>: <issue> (value=<value>)` 行，
     这些行是 ethoinsight.validate.validate_metrics 在 emit_result 时输出的。如果有 VALIDATION_ERROR 行：
     - 收集起来，在 data_quality_warnings 中为每条产出 `{severity: "critical", code: "METRIC_VALIDATION", message: "<metric>: <issue> (value=<value>)",
       evidence: "stdout", blocks_downstream: true}`。**多个 metric 的 VALIDATION_ERROR 要合并**（不要每个 error 一条 warning，
       每个 metric 最多一条 warning，列出该 metric 的所有 issue）。
     - 在 errors 中汇总："N metrics had validation errors (see data_quality_warnings)"
    - 这些错误不阻止你继续跑后续脚本（其他 metric 可能正常），但必须在 handoff 中报告。
     - **不扫 stderr**（stderr 通常是 Python traceback，归 errors 字段处理，不是 validation 路径）
...
5.5. **catalog-driven 验证**：所有 compute 脚本跑完后、封存 handoff 前，调：
     bash `python -m ethoinsight.validate_catalog --plan /mnt/user-data/workspace/plan_metrics.json`
     该命令输出 `VALIDATION_ERROR:` 行到 stdout（与 compute 脚本同格式），你的现有抓取逻辑会把它们
     和 compute 脚本的 VALIDATION_ERROR 行一起汇总进 data_quality_warnings（code=METRIC_VALIDATION）。
     即使没有 violation，也应执行此步骤——它同时是"所有 metric 输出文件都存在"的完整性验证。
6. **封存 handoff（最后一步，完成标志）**：完成全部 `m_*.json` + `validate_catalog` 后，最后一步发出 `seal_code_executor_handoff` 的 tool_call，传入 status/summary/paradigm/
    ...
    工具自动写入 `/mnt/user-data/workspace/handoff_code_executor.json` 并落 manifest hash，返回 OK 即任务完成。
    handoff 落库统一走本工具，不在本 step 外另写 handoff 文件。
...
- 批量并行：bash -c "python -m ethoinsight.scripts... & python -m ethoinsight.scripts... & wait; echo ALL_DONE_<id>"（同指标不同 subject 并行，减少 bash 调用次数）
