...
5. **调 prep_chart_plan 工具生成 plan_charts.json**（确定性入口，取代 bash 拼 catalog.resolve）:
   ...
   - 工具返回 plan_summary：chart_count / fallback_count / skipped_count / chart_ids / column_aliases_applied / groups_applied
...
8. **执行绘图脚本**（最多 4 个）: 遍历 plan_charts.json 的 `charts[]` 数组，对每个 entry 执行 bash `python -m <entry.script> <entry.args 用空格拼接>`。
...
<bash_constraints>
bash 预算最多 5 次（1 次 batch read cat + 最多 4 次绘图脚本）。
允许的命令形式：
