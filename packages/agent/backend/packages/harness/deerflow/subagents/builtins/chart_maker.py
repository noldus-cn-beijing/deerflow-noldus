...
8. **执行绘图脚本**（遍历 plan_charts.json 的 charts[] 全部，已按 output_mode 预算优先级筛过）。
...
<bash_constraints>
bash 预算最多 ~10 次（上下文走逐个 read_file 不占 bash；绘图脚本 = charts[] 条数，由 prep_chart_plan 的 chart_budget 控制总量，通常 ≤ 8）。
允许的命令形式：
