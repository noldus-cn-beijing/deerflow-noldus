...
4. **读 plan_metrics.json 中的 inputs**: read_file `/mnt/user-data/workspace/plan_metrics.json` → 里面 `inputs.raw_files` 就是 raw 轨迹文件路径列表（list of str），直接复用，**不用自己猜**
...
11. **写 handoff_chart_maker.json**: write_file `/mnt/user-data/workspace/handoff_chart_maker.json`，包含 chart_files[]（指向 outputs/ 的 png 路径）、failed_charts[]、paradigm、summary
