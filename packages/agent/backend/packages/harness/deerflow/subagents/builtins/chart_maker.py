...
10. **决策树（按 ethoinsight-chart-maker skill 指示）**:
   - catalog 中有对应图表 → 执行对应绘图脚本（catalog 路径）
   - catalog 无对应但指标数据存在 → 执行 fallback 通用绘图脚本
   - 指标数据缺失或脚本报错 → 记入 failed_charts[]，继续处理下一个图表
11. **执行绘图脚本**（最多 4 个）: bash `python -m ethoinsight.scripts.<paradigm>.<chart_script> --input <metrics_output> --output /mnt/user-data/outputs/<chart_filename>.png`。**注意 --output 必须是 /mnt/user-data/outputs/ 下的路径**，不要写到 workspace/。
12. **写 handoff_chart_maker.json**: write_file `/mnt/user-data/workspace/handoff_chart_maker.json`，包含 chart_files[]（指向 outputs/ 的 png 路径）、failed_charts[]、paradigm、summary
