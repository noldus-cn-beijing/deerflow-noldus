...
9. **据 run_chart_plan 返回值收尾**（run_chart_plan 内部已 seal handoff，**不要再调 seal_chart_maker_handoff**——避免双 seal）。
    ...
    - 返回 status=partial → 有图失败，把 failures 里的失败 chart（chart_id + reason）据实向 lead 汇报（reason 是工具核磁盘/脚本 stderr 的机读真相，直接引用）。
    - 返回 status=failed → 全失败，向 lead 报错说明原因。
    - run_chart_plan 已自动过 chart-maker 对账门（2.2 aggregate must_have 核磁盘），chart_files 是磁盘真相，无须你再核对。remaining_charts（P5 预算降级指纹）已由工具从 plan.charts_budget_remaining 自动透传进 handoff。
10. **present_files**: present_files `/mnt/user-data/outputs/*.png`（run_chart_plan 已画完落盘的），让用户看到生成的图表（present_files 只接受 outputs/ 路径的文件）
