...
4. **Batch read 数据文件（E2E 加速）**: 一次性 cat handoff + plan 到临时文件再读：
   ...
   bash cat /mnt/user-data/workspace/handoff_code_executor.json \
            ...
            > /tmp/cm_context_bundle.txt
   ```
   然后 read_file /tmp/cm_context_bundle.txt 一次拿到全部上下文。
...
11. **封存 handoff**: 调 seal_chart_maker_handoff tool，传入 paradigm/summary/chart_files/failed_charts/status/gate_signals，
