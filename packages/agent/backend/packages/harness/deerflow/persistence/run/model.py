...
class RunRow(Base):
    ...
    # Per-model token breakdown. ``NOT NULL DEFAULT '{}'`` aligns with the
    # upstream ``0001_baseline`` schema (and ``0002_runs_token_usage`` for
    # legacy DBs) so ``safe_add_column`` sees no drift. Read path coalesces
    # with ``or {}`` defensively.
