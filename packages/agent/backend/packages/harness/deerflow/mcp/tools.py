...
# Noldus 定制: MCP 工具返回上限 (与 web_fetch 对齐, 防止大段日志/HTML
# 撑爆 lead agent 上下文)
...
async def get_mcp_tools() -> list[BaseTool]:
    ...
    try:
        ...
        # Noldus 定制: 4096 字符截断 (与 web_fetch 对齐, 防止 MCP 工具返回
        # 大段日志/HTML 撑爆 lead agent 上下文)
        wrapped_tools = [_wrap_tool_with_truncation(t) for t in wrapped_tools]
