...
# token 启发式对命名规整的图有效，但漏掉一批 chart_id/script 不含任何 token 的图
# （如 zone_entry_distribution），它们会被前端 box/bar filter 精确匹配永久吞掉。
# 按 chart_id 显式归类——证据来自 ethoinsight/charts.py 实际绘制调用，不是猜。
# 与 app/gateway/routers/artifacts.py:_derive_chart_type 同型，改时两处同步（SSOT 不漂移）。
...
def _derive_chart_type(chart_id: str, script: str) -> str | None:
    ...
    不在前端各猜一遍。优先查显式 ``_CHART_TYPE_BY_ID``（修 token 漏掉的图名），
    未命中再退 token 启发式（命名约定 ``plot_<...>_<type>``）。
    """
