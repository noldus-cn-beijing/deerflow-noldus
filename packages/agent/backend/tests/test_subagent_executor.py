...
_MOCKED_MODULE_NAMES = [
    ...
    # Noldus fork: these middleware submodules are imported transitively when
    # deerflow.tools (pulled in by tool_search inside _build_initial_state)
    # loads. Upstream's middlewares package has no such submodules, so the flat
    # MagicMock above is sufficient there; the Noldus layout needs them stubbed
    # explicitly or `from deerflow.agents.middlewares.<x> import ...` raises
    # "'deerflow.agents.middlewares' is not a package".
    "deerflow.agents.middlewares.experiment_context",
    "deerflow.agents.middlewares.loop_detection_middleware",
    "deerflow.agents.middlewares.seal_gate_middleware",
...
class TestAgentConstruction:
    ...
    @pytest.mark.skip(
        reason="Noldus fork divergence (out of 债 B scope): asserts app_config flows into "
        "create_chat_model(app_config=...) and build_subagent_runtime_middlewares(...). "
        "The local _create_agent/_build_middlewares signatures don't thread app_config "
        "into the model/middleware factories the upstream way. This is upstream's "
        "broader app_config end-to-end plumbing, separate from the async skill-loading "
        "(async _load_skills / _load_skill_messages / _build_initial_state) that 债 B "
        "delivers. Spec 四.2 surgical skip."
    )
...
class TestSyncExecutionPath:
    ...
    @pytest.mark.skip(
        reason="Noldus fork divergence (out of 债 B scope): references "
        "executor._execute_in_isolated_loop, a method the local Noldus executor does "
        "not have — Noldus uses a different sync-execution implementation. This tests "
        "upstream's isolated-event-loop plumbing, not the async skill-loading "
        "(async _load_skills / _load_skill_messages / _build_initial_state) that 债 B "
        "delivers. Spec 四.2 surgical skip."
    )
    @pytest.mark.anyio
