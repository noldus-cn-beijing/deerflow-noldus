...
# （判据见 executor.py 的 _HANDOFF_CONTENT_CHECKS）。"handoff 存在且可用"的 fixture
# 必须写满足契约的最小合法内容，不能用空 "{}"（空内容会被正确判 FAILED）。
...
class TestSealResumeRecovery:
    ...
    def test_resume_recovers_writes_handoff(self, tmp_path: Path):
        ...
        async def mock_astream_success(*args, **kwargs):
            ...
            # 补轮"成功"= 产出核心字段非空的可用 handoff（Sprint 5.5 内容校验要求）。
            (Path(ws) / "handoff_data_analyst.json").write_text('{"key_findings": ["recovered-finding"]}', encoding="utf-8")
            yield {"messages": [ai_msg, seal_ai]}
