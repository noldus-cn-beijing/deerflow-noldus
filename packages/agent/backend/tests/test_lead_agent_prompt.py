...
@pytest.mark.skip(reason="Noldus-only: assert '**researcher**: Research agent' depends on upstream _build_subagent_section rendering registry custom-agent descriptions; Noldus _build_subagent_section renders a fixed EthoInsight capability list instead. The has_noldus_agents check is a Noldus-only path upstream lacks.")
...
@pytest.mark.skip(reason="Noldus-only: assert depends on upstream-only user-resolution path (get_effective_user_id); local _get_memory_context resolves user via get_current_user instead. app_config plumbing itself is verified by other tests.")
...
@pytest.mark.skip(reason="Noldus-only: SYSTEM_PROMPT_TEMPLATE is a separate Noldus-localized template that intentionally does not carry upstream's 'File Editing Workflow'/'str_replace'/'append=True' section. Placeholder structure also differs (see test_system_prompt_template_preserves_placeholders).")
...
@pytest.mark.skip(reason="Noldus-only: local SYSTEM_PROMPT_TEMPLATE uses a different placeholder set ({memory_context}/{prior_corrections_context}/{resolved_facts_context}/{orchestration_guide}, no {skills_section}/{self_update_section}). apply_prompt_template passes extra kwargs that .format() ignores, so no runtime crash. Verifying Noldus placeholders would require a separate local test.")
