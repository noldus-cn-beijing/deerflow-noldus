...
class TestLoopDetection:
    ...
    def test_warn_only_queued_once_per_hash(self):
        ...
        # NOTE (Noldus fork): tool_freq_warn/hard_limit defaults are 3/5 (vs upstream 30/50);
        # override here so this hash-detection test is not entangled with frequency detection.
        mw = LoopDetectionMiddleware(warn_threshold=3, hard_limit=10, tool_freq_warn=30, tool_freq_hard_limit=50)
        runtime = _make_runtime()
        call = [_bash_call("ls")]
    ...
    def test_different_calls_dont_trigger(self):
        # NOTE (Noldus fork): override tool_freq_warn/hard_limit defaults (3/5)
        # so 10 different bash() calls do not trip frequency detection.
        mw = LoopDetectionMiddleware(warn_threshold=2, tool_freq_warn=30, tool_freq_hard_limit=50)
        runtime = _make_runtime()
    ...
    def test_window_sliding(self):
        # NOTE (Noldus fork): override tool_freq_warn/hard_limit defaults (3/5)
        # so window-sliding (5 different `other_*` calls + 1 ls) does not trip frequency detection.
        mw = LoopDetectionMiddleware(warn_threshold=3, window_size=5, tool_freq_warn=30, tool_freq_hard_limit=50)
        runtime = _make_runtime()
        call = [_bash_call("ls")]
