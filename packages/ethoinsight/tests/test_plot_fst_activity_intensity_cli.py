"""FST activity_intensity CLI test: single file with velocity column."""
...
@pytest.fixture
def fst_velocity_file(tmp_path: Path) -> Path:
    ...
    p = tmp_path / "fst_velocity.txt"
    _df_to_ethovision_file(df, p, subject="Subject 1")
    return p


def test_activity_intensity_cli_writes_png(fst_velocity_file: Path, tmp_path: Path):
    output = tmp_path / "activity_intensity.png"
    result = subprocess.run(
        [sys.executable, "-m", "ethoinsight.scripts.fst.plot_activity_intensity",
         "--input", str(fst_velocity_file), "--output", str(output)],
