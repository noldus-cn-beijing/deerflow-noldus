"""LDB box_light (orphan) + bar_light + trajectory + heatmap CLI tests."""
...
def _make_ldb_df(n: int = 60) -> pd.DataFrame:
...
def test_box_light_n3(three_files: list[Path], tmp_path: Path):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "ethoinsight.scripts.ldb.plot_box_light",
...
def test_bar_light_single(single_file: Path, tmp_path: Path):
    inputs_json = tmp_path / "inputs.json"
    inputs_json.write_text(json.dumps([str(single_file)]))
    output = tmp_path / "bar.png"
    result = subprocess.run(
        [sys.executable, "-m", "ethoinsight.scripts.ldb.plot_bar_light",
...
def test_bar_light_n3(three_files: list[Path], tmp_path: Path):
    inputs_json = tmp_path / "inputs.json"
    inputs_json.write_text(json.dumps([str(p) for p in three_files]))
    groups_json = tmp_path / "groups.json"
    groups_json.write_text(json.dumps({"ctrl": ["Subject 1", "Subject 2"], "tx": ["Subject 3"]}))
...
def test_trajectory_ldb(single_file: Path, tmp_path: Path):
...
def test_heatmap_ldb(single_file: Path, tmp_path: Path):
