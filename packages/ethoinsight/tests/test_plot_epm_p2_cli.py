"""EPM trajectory + heatmap CLI tests (P2)."""
...
@pytest.fixture
def epm_file(tmp_path: Path) -> Path:
    ...
    df = pd.DataFrame({
        ...
        "in_zone_open_arms_1__center_point_": np.where(np.arange(n) < 15, 1, 0),
    })
    p = tmp_path / "epm.txt"
...
def test_heatmap_epm(epm_file: Path, tmp_path: Path):
