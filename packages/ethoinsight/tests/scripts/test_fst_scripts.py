"""Subprocess-level tests for ethoinsight.scripts.fst.*."""
...
class TestComputeImmobilityTime:
    def test_happy_path_writes_json_and_emits_result(
        self, fst_trajectory_file: Path, tmp_path: Path
    ):
        out_path = tmp_path / "metric.json"
        result = _run_script(
            "ethoinsight.scripts.fst.compute_immobility_time",
            ["--input", str(fst_trajectory_file), "--output", str(out_path)],
        )
    ...
    def test_file_without_mobility_column_returns_value_none(
        self, tmp_path: Path, make_fst_df
    ):
        from tests.scripts.conftest import _df_to_ethovision_file

        # Build a df without Mobility_State or any mobility/activity column
        df = make_fst_df()
        df = df.drop(columns=["Mobility_State"])
        path = tmp_path / "no_mobility.txt"
...
class TestComputeImmobilityLatency:
    def test_happy_path(self, fst_trajectory_file: Path, tmp_path: Path):
        out_path = tmp_path / "metric.json"
        result = _run_script(
            "ethoinsight.scripts.fst.compute_immobility_latency",
            ["--input", str(fst_trajectory_file), "--output", str(out_path)],
        )
        assert result.returncode == 0, f"stderr: {result.stderr}"
        payload = json.loads(out_path.read_text())
...
class TestComputeImmobilityBoutCount:
    def test_happy_path(self, fst_trajectory_file: Path, tmp_path: Path):
        out_path = tmp_path / "metric.json"
        result = _run_script(
            "ethoinsight.scripts.fst.compute_immobility_bout_count",
            ["--input", str(fst_trajectory_file), "--output", str(out_path)],
        )
        assert result.returncode == 0, f"stderr: {result.stderr}"
        payload = json.loads(out_path.read_text())
...
class TestPlotBoxImmobility:
    def test_plot_with_groups(self, fst_trajectory_files: list[Path], tmp_path: Path):
        inputs_file = tmp_path / "inputs.json"
        inputs_file.write_text(json.dumps([str(p) for p in fst_trajectory_files]))
        ...
        result = _run_script(
            "ethoinsight.scripts.fst.plot_box_immobility",
...
class TestRunGroupwiseStats:
    def test_stats_with_two_groups(self, fst_trajectory_files: list[Path], tmp_path: Path):
        inputs_file = tmp_path / "inputs.json"
        inputs_file.write_text(json.dumps([str(p) for p in fst_trajectory_files]))
        ...
        result = _run_script(
            "ethoinsight.scripts.fst.run_groupwise_stats",
            ["--inputs", str(inputs_file), "--groups", str(groups_file), "--output", str(out_path)],
        )
        assert result.returncode == 0, f"stderr: {result.stderr}"
        payload = json.loads(out_path.read_text())
