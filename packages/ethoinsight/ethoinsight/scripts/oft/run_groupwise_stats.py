"""OFT: 分组统计检验（Shapiro-Wilk 决策树自动选 t-test / Mann-Whitney）。

CLI: python -m ethoinsight.scripts.oft.run_groupwise_stats \
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = {"paradigm": "open_field", **stats}
    save_output_json(args.output, payload)
    emit_result({"stats": "oft_groupwise", "n_metrics": len(METRICS_TO_TEST),
