...
CLI: python -m ethoinsight.scripts.oft.plot_box_center \
...
METRICS_TO_PLOT = ["center_time_ratio", "center_entry_count"]


def main(argv: list[str] | None = None) -> int:
    args = make_plot_parser(description=__doc__, supports_groups=True).parse_args(argv)
    if not args.inputs:
        print("error: plot_box_center requires --inputs (multi-file)", file=sys.stderr)
        return 2
    ...
    metrics = compute_paradigm_metrics(parsed, paradigm="open_field", groups=groups, zone_overrides=parse_parameters(args) or None)

    output_path = box_plot(
        metrics, metrics_to_plot=METRICS_TO_PLOT, output_path=args.output
    )
    emit_result({"plot": "box_center", "path": output_path, "metrics": METRICS_TO_PLOT})
    return 0
