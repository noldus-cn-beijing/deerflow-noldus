...
CLI: python -m ethoinsight.scripts.epm.plot_open_arm_time_ratio_bar \
       --input <轨迹文件> --output <png>

单样本场景:画当前 subject 的 open_arm_time_ratio 柱状条。
组间场景请改用 plot_box_open_arm。
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.input:
        print("error: plot_open_arm_time_ratio_bar requires --input (single subject)", file=sys.stderr)
        return 2
