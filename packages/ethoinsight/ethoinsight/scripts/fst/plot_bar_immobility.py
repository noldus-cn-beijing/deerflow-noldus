"""FST: 不动行为均值±SEM 柱状图（n<3 自动隐藏误差线）。

CLI: python -m ethoinsight.scripts.fst.plot_bar_immobility \
...
METRICS_TO_PLOT = ["immobility_time", "immobility_bout_count", "immobility_latency"]


def main(argv: list[str] | None = None) -> int:
    args = make_plot_parser(description=__doc__, supports_groups=True).parse_args(argv)
    if not args.inputs:
        print("error: plot_bar_immobility requires --inputs (multi-file)", file=sys.stderr)
        return 2
    ...
    emit_result({"plot": "bar_immobility", "path": output_path, "metrics": METRICS_TO_PLOT})
    return 0
