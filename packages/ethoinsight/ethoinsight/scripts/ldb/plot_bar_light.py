"""LDB: 明暗箱指标均值±SEM 柱状图（n<3 自动隐藏误差线）。

CLI: python -m ethoinsight.scripts.ldb.plot_bar_light \
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.inputs:
        print("error: plot_bar_light requires --inputs (multi-file)", file=sys.stderr)
        return 2
