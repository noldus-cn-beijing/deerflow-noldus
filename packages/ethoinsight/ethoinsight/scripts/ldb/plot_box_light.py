...
CLI: python -m ethoinsight.scripts.ldb.plot_box_light \
...
METRICS_TO_PLOT = ["light_time_ratio", "transition_count"]


def main(argv: list[str] | None = None) -> int:
    args = make_plot_parser(description=__doc__, supports_groups=True).parse_args(argv)
    if not args.inputs:
        print("error: plot_box_light requires --inputs (multi-file)", file=sys.stderr)
        return 2
    ...
    metrics = compute_paradigm_metrics(parsed, paradigm="light_dark_box", groups=groups, zone_overrides=parse_parameters(args) or None)
