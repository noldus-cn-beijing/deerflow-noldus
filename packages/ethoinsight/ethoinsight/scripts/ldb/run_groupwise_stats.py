"""LDB: 分组统计检验（Shapiro-Wilk 决策树自动选 t-test / Mann-Whitney）。

CLI: python -m ethoinsight.scripts.ldb.run_groupwise_stats \
...
def main(argv: list[str] | None = None) -> int:
    ...
    metrics = compute_paradigm_metrics(
        parsed, paradigm="light_dark_box", groups=groups, zone_overrides=zone_overrides
    )
    stats = compare_groups(
        metrics, metrics_to_test=METRICS_TO_TEST, subject_label_map=subject_label_map
    )

    payload = {"paradigm": "light_dark_box", **stats}
