...
- **当前 v0.1 行为学分析方向的唯一硬阻塞**：**Golden Cases**（微调 benchmark + 回归种子 + SFT 评分基准，[Issue #90](https://github.com/noldus-cn-beijing/noldus-insight/issues/90)，OPEN），卡**微调路线**、需行为学同事产出对/错标准；不卡 v0.1 端到端分析。**结构聚合（原阻塞 [Issue #98](https://github.com/noldus-cn-beijing/noldus-insight/issues/98)）已于 2026-06-18 CLOSED**——同事方法论已交付、聚合机制已在线，功能性上 v0.1 六范式端到端分析现已可用（识别+判读+聚合全齐）。详见 [docs/milestone/blocked-on-expert-methodology.md](docs/milestone/blocked-on-expert-methodology.md)。harness/基础设施层均无此阻塞，可独立推进。
...
5. **微调方案已锁定** — 后训练基座 = **Qwen3.6-35B-A3B**（2026-06-30 拍板，HF: Qwen/Qwen3.6-35B-A3B；候选 Qwen3.5-35B-A3B 同构仅作退路）。路线：SkillOpt 优化 skill → 生成高质量 SFT 轨迹 → SFT（LoRA）→ GRPO 强化学习。后训练平台候选火山引擎 verl / Fireworks，客户硬件 RTX 5090 32GB。⚠️ **本条历史曾写 Qwen3-8B Dense / Qwen3-30B-A3B-Instruct-2507，已过时作废**；基座选型理由 + 架构风险（Gated DeltaNet 训练时支持需先冒烟）+ 3.5 退路，SSOT 见 memory `project_base_model_qwen36_35b_a3b`，老 memo [docs/plans/2026-05-13-base-model-decision-memo.md](docs/plans/2026-05-13-base-model-decision-memo.md) 仅作历史记录。
...
14. **Skill 优化 → SFT 路线（2026-06-04 启动）** — 采用微软 SkillOpt 方法论：行为学专家产出 Golden Cases（eval benchmark）→ 改造 SkillOpt 代码（自定义 EnvAdapter）→ 跑优化循环产出 best_skill.md → 用优化后的 skill 驱动 agent 生成高质量 SFT 轨迹 → 微调 Qwen3.6-35B-A3B（基座见第 5 条）。详见 [docs/plans/2026-06-04-skillopt-skill-optimization-plan.md](docs/plans/2026-06-04-skillopt-skill-optimization-plan.md)。**当前阻塞项：行为学专家 Golden Cases 待产出。**
...
3. [docs/milestone/blocked-on-expert-methodology.md](docs/milestone/blocked-on-expert-methodology.md) — 行为学分析方向当前唯一硬阻塞 Golden Cases（[Issue #90](https://github.com/noldus-cn-beijing/noldus-insight/issues/90)）的精确待办；结构聚合（#98）已解除，附对接记录
...
- [docs/milestone/blocked-on-expert-methodology.md](docs/milestone/blocked-on-expert-methodology.md) — 唯一硬阻塞 Golden Cases（[Issue #90](https://github.com/noldus-cn-beijing/noldus-insight/issues/90)）待办；结构聚合（#98）已 CLOSED 解除
