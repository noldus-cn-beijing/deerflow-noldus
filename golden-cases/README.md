...
| Case 编号 | 范式 | 目录 | `paradigm` 字段填什么（canonical name） |
|-----------|------|------|------|
| case-001 | **EPM**（高架十字迷宫） | `case-001-epm-baseline/` | `epm` |
| case-002 | **OFT**（旷场） | `case-002-oft-baseline/` | `open_field` |
| case-003 | **LDB**（明暗箱） | `case-003-ldb-baseline/` | `light_dark_box` |
| case-004 | **FST**（强迫游泳） | `case-004-fst-baseline/` | `forced_swim` |
| case-005 | **Zero Maze**（零迷宫） | `case-005-zero-maze-baseline/` | `zero_maze` |
| case-006 | **TST**（悬尾实验） | `case-006-tst-baseline/` | `tail_suspension` |

> `metadata.yaml` 和 `expected-analysis.yaml` 里的 `paradigm` 字段，模板已经帮您填好 canonical name 了，**您不用改这一行**。列在这里只是让您知道每个目录对应哪个范式。
...
每个指标只需要填 `expected_range`（您认为合理的数值范围）。指标名和所属组（`group`）已经帮您填好了。
...
> 范围是按**组**给的（control 组 / treatment 组各一个范围），不是按单只动物——因为判读看的是"两组有没有显著差异"，不是某只动物的绝对值对不对。
